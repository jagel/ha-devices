# Momentary Garage Switch - Home Assistant Custom Integration

A production-ready, reusable Home Assistant custom integration for controlling garage doors with momentary switches and binary sensors.

## Features

- ✅ **Momentary Switch Control**: Automatically pulses a physical switch (on → wait → off)
...
| `momentary_duration` | No | 1 | How long to keep the switch on (0.1-60 seconds) |
...
```
┌─────────────────────────────────────────────────────────┐
│  MomentaryGarageSwitch Entity                           │
│  - Shows "open"/"close" based on binary sensor         │
│  - Triggers momentary pulse when toggled                │
└────────────┬────────────────────────────┬───────────────┘
             │                            │
   ┌─────────▼──────────┐      ┌─────────▼──────────┐
   │ StateTracker       │      │ MomentaryHandler   │
...
4. **MomentaryHandler** triggers pulse:
   - Turn on `trigger_switch`
   - Wait for `momentary_duration` seconds
   - Turn off `trigger_switch`

5. **Physical garage door** responds:
   - Receives momentary pulse
...
### Using MomentaryHandler in Your Integration

```python
from custom_components.momentary_garage_switch.helpers import MomentaryHandler

class MyCustomSwitch(SwitchEntity):
    def __init__(self, hass):
        self._momentary = MomentaryHandler(hass)
    
    async def async_turn_on(self, **kwargs):
        # Trigger a 2-second pulse
        await self._momentary.trigger_momentary(
            "switch.my_device", 
            duration=2.0
        )
```
...
```python
from custom_components.momentary_garage_switch.helpers import StateTracker
...
1. Check logs for errors:
   ```bash
   grep "momentary_garage_switch" /config/home-assistant.log
   ```
...
2. **Check logs**:
   ```
   [custom_components.momentary_garage_switch.helpers.state_tracker] State changed
   ```
...
2. **Duration too short**:
   ...
   - Try increasing `momentary_duration` to 1.5 or 2 seconds

3. **Check logs**:
   ```
   [custom_components.momentary_garage_switch.helpers.momentary_handler] Triggering momentary switch
   ```
...
└── helpers/
    ...
    ├── momentary_handler.py # Reusable momentary logic
...
- Home Assistant Community: Tag with `momentary_garage_switch`
